...
// @version      2.10.0
// @description  Sub2 account health, API-key-named route history, reliability events, manual upstream balance queries, and protected controls (no active probing).
...
(function (factory) {
  ...
  //   GET  /api/v1/admin/users/:id/api-keys             请求历史中的客户端 Key 名称
  ...
  const SUB2_API_KEY_PAGE_SIZE = 100;
  const SUB2_API_KEY_LOOKUP_CONCURRENCY = 8;
  const SUB2_API_KEY_QUERY_TIMEOUT_MS = 8000;
  ...
  function sub2ExtractApiKeyName(requestItem) {
    const rawApiKeyName = requestItem?.api_key_name
      ?? requestItem?.apiKeyName
      ?? requestItem?.key_name
      ?? requestItem?.api_key?.name
      ?? requestItem?.apiKey?.name;
    return typeof rawApiKeyName === 'string' ? rawApiKeyName.trim() : '';
  }
  ...
  function sub2NormalizeRequestHistory(payload) {
    ...
    for (const requestItem of sub2GetPaginatedItems(payload)) {
      ...
      normalizedRequests.push({
        ...
        apiKeyName: sub2ExtractApiKeyName(requestItem),
        createdAt,
  ...
  async function sub2FetchUserApiKeyNames(userId) {
    const numericUserId = Number(userId);
    if (!Number.isInteger(numericUserId) || numericUserId <= 0) return new Map();

    const apiKeyNamesById = new Map();
    for (let page = 1; ; page += 1) {
      const data = await sub2ApiRequestWithTimeout(
        'GET',
        `/admin/users/${numericUserId}/api-keys?page=${page}&page_size=${SUB2_API_KEY_PAGE_SIZE}`,
        undefined,
        SUB2_API_KEY_QUERY_TIMEOUT_MS,
      );
      const items = sub2GetPaginatedItems(data);
      for (const apiKeyRecord of items) {
        const numericApiKeyId = Number(apiKeyRecord?.id);
        if (!Number.isInteger(numericApiKeyId) || numericApiKeyId <= 0) continue;
        const apiKeyName = typeof apiKeyRecord?.name === 'string'
          ? apiKeyRecord.name.trim()
          : '';
        apiKeyNamesById.set(numericApiKeyId, apiKeyName);
      }
      const total = Number(data?.total);
      if (!items.length
        || items.length < SUB2_API_KEY_PAGE_SIZE
        || (Number.isFinite(total) && apiKeyNamesById.size >= total)) {
        return apiKeyNamesById;
      }
    }
  }

  async function sub2EnrichRequestHistoryWithApiKeyNames(requestHistory, apiKeyNamesById = new Map()) {
    const normalizedRequestHistory = Array.isArray(requestHistory) ? requestHistory : [];
    const apiKeyNameCache = apiKeyNamesById instanceof Map ? apiKeyNamesById : new Map();
    const requestedApiKeyIdsByUserId = new Map();

    for (const requestItem of normalizedRequestHistory) {
      const numericApiKeyId = Number(requestItem?.apiKeyId);
      if (!Number.isInteger(numericApiKeyId) || numericApiKeyId <= 0) continue;

      const inlineApiKeyName = sub2ExtractApiKeyName(requestItem);
      if (inlineApiKeyName) {
        apiKeyNameCache.set(numericApiKeyId, inlineApiKeyName);
        continue;
      }
      if (apiKeyNameCache.has(numericApiKeyId)) continue;

      const numericUserId = Number(requestItem?.userId);
      if (!Number.isInteger(numericUserId) || numericUserId <= 0) continue;
      if (!requestedApiKeyIdsByUserId.has(numericUserId)) {
        requestedApiKeyIdsByUserId.set(numericUserId, new Set());
      }
      requestedApiKeyIdsByUserId.get(numericUserId).add(numericApiKeyId);
    }

    const userIds = [...requestedApiKeyIdsByUserId.keys()];
    for (let offset = 0; offset < userIds.length; offset += SUB2_API_KEY_LOOKUP_CONCURRENCY) {
      const userIdBatch = userIds.slice(offset, offset + SUB2_API_KEY_LOOKUP_CONCURRENCY);
      const batchResults = await Promise.allSettled(
        userIdBatch.map((userId) => sub2FetchUserApiKeyNames(userId)),
      );
      batchResults.forEach((result, index) => {
        if (result.status !== 'fulfilled') return;
        const userId = userIdBatch[index];
        const requestedApiKeyIds = requestedApiKeyIdsByUserId.get(userId) || new Set();
        for (const apiKeyId of requestedApiKeyIds) {
          apiKeyNameCache.set(apiKeyId, result.value.get(apiKeyId) || '');
        }
        for (const [apiKeyId, apiKeyName] of result.value) {
          apiKeyNameCache.set(apiKeyId, apiKeyName);
        }
      });
    }

    return normalizedRequestHistory.map((requestItem) => {
      const numericApiKeyId = Number(requestItem?.apiKeyId);
      const apiKeyName = Number.isInteger(numericApiKeyId) && numericApiKeyId > 0
        ? sub2ExtractApiKeyName(requestItem) || apiKeyNameCache.get(numericApiKeyId) || ''
        : sub2ExtractApiKeyName(requestItem);
      return requestItem.apiKeyName === apiKeyName
        ? requestItem
        : { ...requestItem, apiKeyName };
    });
  }
  ...
  class Sub2Controller {
    ...
    async refreshRoutingActivity(accountRequestSequence) {
      ...
      const requestHistoryWithApiKeyNames = await sub2EnrichRequestHistoryWithApiKeyNames(
        rawRequestHistory,
        this.apiKeyNamesById,
      );
      if (
        routingRequestSequence !== this.routingRequestSequence
        || accountRequestSequence !== this.refreshRequestSequence
      ...
      this.requestHistory = sub2EnrichRequestHistoryWithTTFT(
        requestHistoryWithApiKeyNames,
        this.ttftSnapshot,
      );
    ...
    renderRequestHistoryCard(now) {
      ...
      for (const requestItem of matchingRequests) {
        ...
        const requestApiKeyLabel = requestItem.apiKeyName
          ? `Key ${requestItem.apiKeyName}`
          : requestItem.apiKeyId
            ? `Key #${requestItem.apiKeyId}`
            : '';
    ...
    renderDiagnosticsDrawer() {
      ...
      const requestGroup = diagnosticsRequest.groupId
        ...
        : null;
      const diagnosticsApiKeyLabel = diagnosticsRequest.apiKeyName
        ? `Key ${diagnosticsRequest.apiKeyName}`
        : diagnosticsRequest.apiKeyId
          ? `Key #${diagnosticsRequest.apiKeyId}`
          : '';
      const factValues = [
